...
from src.tests.fixtures.token import VALID_USER_NAME, VALID_USER_PASSWORD
...
def test_login(test_user, client):
    response = client.post(
        "/v1/login/",
        data={"username": VALID_USER_NAME, "password": VALID_USER_PASSWORD},
    )
    ...
    payload = jwt.decode(
        token.access_token, settings.jwt_secret_key, algorithms=[settings.jwt_algorithm]
    )
